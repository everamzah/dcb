...
minetest.after(0, function()
        local inv = minetest.create_detached_inventory("creative", {
                allow_move = function(inv, from_list, from_index, to_list, to_index, count, player)
                        if minetest.check_player_privs(player:get_player_name(), {creative=true}) then --minetest.setting_getbool("creative_mode") then
                                return count
                        else
                                return 0
                        end
                ...
                on_take = function(inv, listname, index, stack, player)
                        ...
                        if stack then
				--return
				--player:get_inventory():add_item('main', stack:get_name().." "..stack:get_stack_max()) --" 98")
		...
		local maxstacksize = ItemStack(itemstring):get_stack_max()
		--print(maxstacksize)
		inv:add_item("main", ItemStack(itemstring.." "..maxstacksize))
        end
...
creative_inventory.set_creative_formspec = function(player, start_i, pagenum)
	...
	local formspec =
        ...
        --)
	formspec_storage[player] = formspec
	minetest.show_formspec(player, "dcb:creative", formspec)
end

minetest.register_privilege("creative", "creative")

minetest.register_chatcommand("creative", {
	privs = {creative = true},
	func = function(name)
		--local player = get_player_by_name(name)
		creative_inventory.set_creative_formspec(name, 0, 1)
	end
})
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	...
	if formname ~= "dcb:creative" then return end
        -- Figure out current page from formspec
        local current_page = 0
        local formspec = formspec_storage[player:get_player_name()] --player:get_inventory_formspec()
        local start_i = string.match(formspec, "list%[detached:creative;main;[%d.]+,[%d.]+;[%d.]+,[%d.]+;(%d+)%]")
        start_i = tonumber(start_i) or 0

        if fields.creative_prev then
                start_i = start_i - 4*6
		creative_inventory.set_creative_formspec(player:get_player_name(), start_i, start_i / (6*4) + 1)
        end
