...
xdecor.register("letterbox", {
	...
	after_place_node = function(pos, placer, _)
		...
		meta:set_string("infotext", owner.."'s Letterbox")
	...
	on_rightclick = function(pos, _, clicker, _)
		...
		if owner == player then
			minetest.show_formspec(player, "", letterbox.get_formspec(pos))
		else minetest.show_formspec(player, "",
				letterbox.get_insert_formspec(pos, owner))
		end
	end,
	...
	allow_metadata_inventory_put = function(pos, listname, _, stack, _)
		...
		if listname == "drop" and stack:get_name() == "default:book_written" then
			local meta = minetest.get_meta(pos)
			local inv = meta:get_inventory()
...
function letterbox.get_formspec(pos)
...
function letterbox.get_insert_formspec(pos, owner)
	local spos = pos.x..","..pos.y..","..pos.z
