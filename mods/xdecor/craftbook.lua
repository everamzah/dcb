local craftbook, datas = {}, {}

function craftbook:get_recipe(item)
...
function craftbook:get_formspec(stack, pagenum, item, recipe_num, filter, player_name)
	...
	minetest.show_formspec(player_name, "xdecor:crafting_book", formspec)
end

function craftbook:get_items(filter, player_name)
	local items_list = {}
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	if formname ~= "xdecor:crafting_book" then return end
	...
	if fields.clear then
		craftbook:get_items(nil, player_name)
		craftbook:get_formspec(stack, 1, nil, 1, "", player_name)
	elseif fields.alternate then
		local item = formspec:match("item_image_button%[.*;([%w_:]+);") or 1
		local recipe_num = tonumber(formspec:match("Recipe%s(%d+)")) or 1
		recipe_num = recipe_num + 1
		craftbook:get_formspec(stack, pagenum, item, recipe_num, filter, player_name)
	elseif fields.search then
		local lowstr = fields.filter:lower()
		craftbook:get_items(lowstr, player_name)
		craftbook:get_formspec(stack, 1, nil, 1, lowstr, player_name)
