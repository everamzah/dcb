...
function craftbook:set_formspec(player_name, pagenum, recipe_num)
	...
	minetest.show_formspec(player_name, "xdecor:craftbook", formspec)
end

function craftbook:get_items(player_name)
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	if formname ~= "xdecor:craftbook" then return end
	...
	if fields.clear then
		...
		craftbook:get_items(player_name)
		craftbook:set_formspec(player_name, 1, 1)
	elseif fields.alternate then
		local recipe_num = tonumber(formspec:match("Recipe%s(%d+)")) or 1
		recipe_num = recipe_num + 1
		craftbook:set_formspec(player_name, pagenum, recipe_num)
	...
	elseif fields.prev or fields.next then
		...
		craftbook:set_formspec(player_name, pagenum, 1)
...
minetest.register_craftitem("xdecor:crafting_guide", {
	...
	on_use = function(itemstack, user)
		...
		if not datas[player_name] then
			...
			minetest.show_formspec(player_name, "xdecor:craftbook", datas[player_name].formspec)
		end
	end
})
