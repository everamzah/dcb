function teleporter.getList(name)
	...
	local keys = {};
		...
		keys[#keys+1] = receiver.fav..","..
			receiver.pub..","..
			minetest.formspec_escape(v)..","..
			minetest.formspec_escape(receiver.name)..","..
			minetest.formspec_escape(receiver.desc);
        end
...
function teleporter.getreceiverForm(pos)
	...
	local form = "size[8,5;]"..default.gui_bg..default.gui_bg_img..default.gui_slots..
		"label[0.1,0.0;Receiver Settings]"..
		"box[0.1,0.6;3,0.05;#FFFFFF]"..
		"checkbox[0.1,0.6;public;Public (Server admins only);"..public.."]"..
		"checkbox[0.1,1;favorite;Mark as favorite;"..fav.."]"..
		"field[0.3,2.5;4,1;name;Name:;${name}]"..
		"field[0.3,3.65;8,1;desc;Description:;${desc}]"..
		"button_exit[4.1,4.4;4,1;save;OK]"..
		"button_exit[0.1,4.4;4,1;exit;Cancel]"..
		default.gui_slots;
	return form
end
...
function teleporter.receiver_placed(itemstack, placer, pointed_thing)
	...
	meta:set_string("infotext", "Offline");
	...
	if teleporter.can_access(pos, sender) then
		...
		elseif fields.public then
			...
			if privs["server"] then
				...
				minetest.chat_send_player(sender:get_player_name(),"Only the server admin can add this.")
			end
		...
		minetest.chat_send_player(sender:get_player_name(), "Only "..meta:get_string("owner").." or the admin can change this.")
	end
end
...
function teleporter.change_dest(player,pos,name,desc)
	...
	-- FIXME turn stores into booleans
	if fav == "true" then
		fav = 1;
	else
		fav = 0;
...
function teleporter.getform(pos)
	...
	if mode == 0 or mode == nil then --mode 0 means choose a receiver from the list
		...
		form = "size[10,7;]"..default.gui_bg..default.gui_bg_img..default.gui_slots..
			"label[0.1,0.0;Choose a receiver:]"..
			"box[0.1,0.6;6,0.05;#FFFFFF]"..
			"checkbox[0.1,0.6;public;Show public list;"..public.."]"..
			"label[0.1,1.3;Known Receivers: ]"..
			"tablecolumns[image,tooltip=favorite,1=teleporter_bookmark.png;"..
				"image,tooltip=public receiver,1=teleporter_world.png;text,tooltip=Coordinates,align=right;"..
				"text,tooltip=Name;text,tooltip=Description]"..
			"table[0.1,1.7;9.7,4.5;dest;"..list..";"..item.."]"..
			"button[0.1,6.2;5.7,1;reload;Reload]"..
			"button[6.0,6.2;4.0,1;save_dest;OK]";
	else --mode 1 means the teleporter has a destination, an can be used
		form = "size[9,6;]"..default.gui_bg..default.gui_bg_img..default.gui_slots..
			...
			"button[0.1,5;5.7,1;unlink;Unlink / Choose new]"..
			"button_exit[6.0,5;3.0,1;save;OK]";
	end
...
function teleporter.received_fields(pos, formname, fields, sender)
	...
	if teleporter.can_access(pos,sender) then
		...
		minetest.chat_send_player(sender:get_player_name(),
			"Teleporter -!-: you cannot change the settings, only "..name..
			" or the server admins are alowed to do it!"
		)
	end
end
